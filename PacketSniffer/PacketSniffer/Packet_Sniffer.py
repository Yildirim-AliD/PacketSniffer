...
class PacketSnifferApp(QWidget):
    ...
    def initUI(self):
        ...
        self.start_button = QPushButton("Start Packet Capture")
        self.start_button.clicked.connect(self.start_sniffing)
        self.stop_button = QPushButton("Stop Packet Capture")
        self.stop_button.setEnabled(False)
        self.stop_button.clicked.connect(self.stop_sniffing)
        self.log_button = QPushButton("Record")
        ...
        self.filter_input.setPlaceholderText("Enter Filter (örn. ip or tcp)")
        ...
        self.search_bar.setPlaceholderText("Search within Packets")
        ...
        self.packet_count_label = QLabel("Total Captured Packets: 0")
        self.tcp_label = QLabel("TCP Packets: 0")
        self.udp_label = QLabel("UDP Packets: 0")
        self.icmp_label = QLabel("ICMP Packets: 0")
        ...
        self.packet_save = QPushButton("Save Packets")
    ...
    def start_sniffing(self):
        if not self.sniffer_thread.isRunning():
            ...
            if iface:
                ...
                self.show_error_message("Invalid Network interface selected!")
    ...
    def update_packet_stats(self):
        self.packet_count_label.setText(f"Total Captured Packets: {self.packet_count}")
        self.tcp_label.setText(f"TCP Packet: {self.tcp_count}")
        self.udp_label.setText(f"UDP Packet: {self.udp_count}")
        self.icmp_label.setText(f"ICMP Packet: {self.icmp_count}")
    ...
    def toggle_logging(self):
        if self.log_button.isChecked():
            try:
                ...
                self.log_button.setText("Stop Recording")
            except Exception as e:
                self.show_error_message(f"The recording file could not be opened: {str(e)}")
            ...
            if hasattr(self,"log_file") and self.log_file:
                try:
                    ...
                    self.show_error_message(f"The recording file could not be closed: {str(e)}")
                finally:
                    self.log_file = None
            self.log_button.setText("Record")
    def save_packets(self):
        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getSaveFileName(self,"Save packets","","Text Files (*.txt);;All Files (*)", options=options)
        if file_path:
